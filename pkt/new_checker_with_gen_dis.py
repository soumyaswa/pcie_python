...
class checker_pkt(cfg0_pkt):
    ...
    def checker_fn(self, packet_check):
        ...
        if not (0 <= bdf < 2**8 and 0 <= conf_type <= 1 and 0 <= ep <= 1 and 0 <= td <= 1 and block == 1):
            ...
            packet_check["result"] = "Failed"
        else:
            print("Packet is valid")
            packet_check["result"] = "Passed"

        print(f"Packet: {packet_check}")
        return packet_check
...
with open('output.txt', 'w') as f:
    ...
    for i, packet in enumerate(packets):
        c1 = checker_pkt()
        result_packet = c1.checker_fn(packet)
        packet_results.append(result_packet)

    f.write("Packet results:\n")
    for i, packet in enumerate(packet_results):
        f.write(f"Packet {i}: {packet['result']}\n")
        f.write(json.dumps(packet) + '\n')

    invalid_pkt = [i for i, packet in enumerate(packet_results) if packet["result"] == "Failed"]
    valid_pkt = [i for i, packet in enumerate(packet_results) if packet["result"] == "Passed"]
